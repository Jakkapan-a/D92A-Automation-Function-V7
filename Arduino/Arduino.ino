...
void output_Alarm(int inputAlarm = 0)
{
	/*
	  input = 1 Alarm_Sound On and off all
	  input = 2 Alarm_Green On and off all
	  input = 3 Alarm_Yellow On and off all
	  input = 4 Alarm_Red On and off all
	  input = 11 Alarm_Sound On
	  input = 22 Alarm_Green On
	  input = 33 Alarm_Yellow On
	  input = 44 Alarm_Red On
	*/
	switch (inputAlarm)
	{
	case 1:
...
int buutonPressed = 0;
void func_auto(bool inputState)
{
	// If the input is false, exit the function. If the input is true, run the function.
	if (!inputState) {
		return;
	}
	// sendDataToSerialPort("P:0002");
	...
	case 0:
		...
		else if (timer_count > func_time_next && state_func && state_connected) {
			setNextItem((func_item + 1));
			state_func = false;
		}
		...
		break;
	case 1:
		if (timer_count > func_time_next && !state_func) { // BAT on
			sendDataToSerialPort("set:99"); // Set process bar
			output_Alarm();
			output_DVR();
			Alarm_Yellow.on();
			BAT.on();
	...
	case 2:
		if (timer_count > func_time_next && !state_func) { // ACC on
			...
			setNextItem((func_item + 1), 8);
			state_func = false;
		}
		break;
	case 3:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing and check cuuent 
			int AccCurent = 500;
			int BatCurrent = 500;
			if (BatCurrent < 400 || AccCurent < 400) // Current
			{
				setFail(); // If fail
				return;
			}
			imageProcessing("0001"); // Full Screen
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image)
		{
			if (!the_result_Image) {
				setFail();
				return;
		...
		break;
	case 4:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing

			imageProcessing("0002"); // Record
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
			if (!the_result_Image) {
				if (failCount < 4) {
					failCount++;
					imageProcessing("0002"); // Re Test Icon Record
					setNextItem(func_item, 1);
				}
				else
				{
	...
	case 5:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0003"); //Check Icon Date & Time
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 9:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0004"); // Check Icon LOCK
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 11:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0007"); // Setting Menu
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 12:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0008"); // Setting Menu Icon Focus
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 13:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0009"); // Setting Menu Icon Focus
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 14:
		if (timer_count > func_time_next && !state_func) { // SlN 1 Pressed 
			SLN45_BT1.on(true);
			state_func = true;
			RearCAM_Vplus.on();		// On V+
			RearCAM_Vminus.on();	// On V-
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func) {
	...
	case 15:
		if (timer_count > func_time_next && !state_func) { // SlN 3 Pressed Capture 
			SLN45_BT3.on(true);
			state_func = true;
			setNextItem(func_item);
		}
	...
	case 16:
		if (timer_count > func_time_next && !state_func) { // SlN 2 Pressed switch back camera  
			SLN45_BT2.on(true);
			state_func = true;
	...
	case 17:
		if (timer_count > func_time_next && !state_func) { // SlN 3 Pressed Capture
			SLN45_BT3.on(true);
			state_func = true;
			imageProcessing("0019"); // Setting Menu Icon Focus
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
			if (!the_result_Image) {
		...
		break;
	case 18:
		if (timer_count > func_time_next && !state_func) { // SlN 2 Pressed switch front camera
			SLN45_BT2.on(true);
			state_func = true;
			RearCAM_Vplus.off();	// Off V+
			RearCAM_Vminus.off();	// Off V-
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func) {
			setNextItem((func_item + 1));
	...
	case 23:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0010"); // List Setting Menu
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 24:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0011"); // List Setting Menu Selected Play Volume 1/3
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
		...
		break;
	case 25:
		if (timer_count > func_time_next && !state_func) { // SlN 3 Pressed 1/2
			SLN45_BT3.on(true);
			state_func = true;
			setNextItem(func_item, 2);
		}
	...
	case 26:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0012"); // List Setting Menu Selected Key/Alaram sound 2/3
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
		...
		break;
	case 27:
		if (timer_count > func_time_next && !state_func) { // SlN 3 Pressed 2/2
			SLN45_BT3.on(true);
			state_func = true;
			setNextItem(func_item, 2);
		}
	...
	case 28:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0013"); // List Setting Menu Selected Sound Record 3/3
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
	...
	case 30:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0014"); // Sound Record Off
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
			if (!the_result_Image) {
				failCount++;
				if (failCount < 2) {
					imageProcessing("0015"); // Sound Record On
					setNextItem(func_item);
				}
				else if (failCount > 2)
				{
	...
	case 31:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0017"); // Sound Record Selected Off
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
		...
		break;
	case 32:
		if (timer_count > func_time_next && !state_func) { // SlN 1 Pressed 1/3
			SLN45_BT1.on(true);
			state_func = true;
			setNextItem(func_item, 1);
		}
	...
	case 33:
		...
		break;
	case 34:
		if (timer_count > func_time_next && !state_func) { // SlN 1 Pressed 2/3
			SLN45_BT1.on(true);
			state_func = true;
			setNextItem(func_item, 1);
		}
	...
	case 35:
		...
		break;
	case 36:
		if (timer_count > func_time_next && !state_func) { // SlN 1 Pressed 3/3
			SLN45_BT1.on(true);
			state_func = true;
			setNextItem(func_item, 1);
		}
	...
	case 38:
		if (timer_count > func_time_next && !state_func) { // ImageProcessing
			imageProcessing("0018"); // Recording 15 sec
			state_func = true;
			setNextItem(func_item);
		}
		else if (timer_count > func_time_next && state_func && state_received_Image) {
			if (!the_result_Image) {
				if (failCount < 3) {
					failCount++;
					imageProcessing("0018"); // Recording 15 sec gain
					setNextItem(func_item);
				}
				else
				{
